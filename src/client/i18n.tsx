...
const translations: Record<Language, Record<string, string>> = {
  'en': {
    ...
    'settings.sankeySettings': 'Sankey Settings',
    'settings.sankeyWidthMode': 'Link width metric',
    'settings.sankeyWidthMode.value': 'Value (default — auto-falls-back to events when no value data)',
    'settings.sankeyWidthMode.events': 'Events (always count of events)',
    'settings.sankeyWidthModeHint': 'Sankey link width is normally proportional to the sum of the Value Field per flow (typically bytes). When the Value Field does not exist on incoming messages — e.g. Suricata IDS events — every flow contributes 1 unit, so the width effectively becomes the event count automatically; no setting change needed. Pick "Events" if you have real byte data but explicitly want widths to represent event count instead.',
    'settings.valueFieldHint': 'GELF field whose numeric value is summed per flow (typically bytes / packet length).',
    'settings.valueFieldEventModeHint': 'No length field in your logs (e.g. Suricata IDS events)? Set the field below to a name that does not exist in your messages, leave Default Value at 1, and Transform on None — every event then contributes 1 unit and the dashboard becomes an event-count visualization. Sankey hover already shows the events total.',
  ...
  'zh-TW': {
    ...
    'settings.sankeySettings': '桑基圖設定',
    'settings.sankeyWidthMode': '線寬計算方式',
    'settings.sankeyWidthMode.value': 'Value（預設 — 沒有數值資料時自動改用事件數）',
    'settings.sankeyWidthMode.events': 'Events（永遠以事件數計算）',
    'settings.sankeyWidthModeHint': '桑基圖線段寬度預設等比例於每筆流量「數值欄位」加總（通常是 bytes）。如果訊息上沒有數值欄位（例如 Suricata IDS 事件），每筆流量會貢獻 1 個單位，線寬其實就會自動變成「事件次數」 — 不用改設定。選「Events」是當你有真實的數值資料、但想刻意讓寬度反映事件次數而非位元組量時用。',
    'settings.valueFieldHint': '每筆流量會加總此 GELF 欄位的數值（通常是位元組 / 封包長度）。',
    'settings.valueFieldEventModeHint': '訊息裡沒有長度欄位（例如 Suricata 之類的 IDS 事件）？把下方欄位填一個訊息中不存在的名稱、預設值維持 1、數值轉換選「無」— 每個事件就會貢獻 1 個單位，整個儀表板轉成「事件計數」的視覺化。桑基圖游標停留時的小卡片本來就會顯示事件數。',
